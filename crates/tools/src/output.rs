...
pub fn encode_tool_output(model_output: String, file_change: Option<FileChangeOutput>) -> String {
    let file_changes = file_change.into_iter().collect::<Vec<_>>();
    encode_structured_tool_output(model_output, file_changes, Vec::new())
}
...
pub fn encode_tool_output_with_todos(model_output: String, todos: Vec<TodoItem>) -> String {
    encode_structured_tool_output(model_output, Vec::new(), todos)
}
...
fn encode_structured_tool_output(
    ...
    if file_changes.is_empty() && todos.is_empty() {
        return model_output;
    }
...
fn decode_tool_output(raw_output: String) -> DecodedToolOutput {
    ...
    match serde_json::from_str::<StructuredToolOutput>(json) {
        Ok(output) => DecodedToolOutput {
            ...
            file_changes: output.file_changes,
            todos: output.todos,
        },
